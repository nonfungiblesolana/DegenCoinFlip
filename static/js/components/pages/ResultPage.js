...
const PleaseWaitButton = ({ setShowPleaseWaitButton, isWinner }) => {
    ...
    return (
        <>
            ...
            <ProgressBar animated className="rounded-0" now={counter} max={MAX} min={0} />
        </>
        ...
        return (
            <>
                <h5 className="mt-2 text-success fst-italic">
                    ...
                    {message === constantsMsgs.STILL_WORKING_MSG && <img src="https://i.imgur.com/1pmDwMC.gif" className="img-fluid mb-1" style={{ maxWidth: '24px' }} />}
                </h5>
                <img src={constants.BUTTONS.PleaseWait} alt="please wait..." width="100%" height="100%" />
    ...
    return (
        <div className="play step2 py-5 py-md-0">
            <div className="form-signin">
                ...
                {
                    depositTxn?.length > 0 &&
                    <h6 className="text-secondary mt-2"><small>{depositTxn?.slice(0, 4)}</small></h6>
                }
                {
                    isWinner &&
